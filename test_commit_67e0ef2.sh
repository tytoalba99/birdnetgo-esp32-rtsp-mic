...
if [[ "$shift_val" =~ ^[0-9]+$ ]] && (( shift_val >= 0 && shift_val <= 24 )); then
...
if echo "$R" | grep -qF '"ok":true'; then
    ...
    fail "server_start after idle cpu_freq change failed: $R"
fi
...
if [[ -n "$ORIG_TOPIC2" ]]; then
    ENCODED_TOPIC=$(printf '%s' "$ORIG_TOPIC2" | python3 -c "import sys, urllib.parse; print(urllib.parse.quote(sys.stdin.read().rstrip('\n')))")
    set_key "mqtt_topic" "$ENCODED_TOPIC" > /dev/null || true
fi
...
MDNS_TMP=$(mktemp)
dns-sd -G v4 esp32mic.local > "$MDNS_TMP" 2>/dev/null &
DNS_PID=$!
sleep 2
kill "$DNS_PID" 2>/dev/null || true
wait "$DNS_PID" 2>/dev/null || true
MDNS_IP=$(grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' "$MDNS_TMP" | head -1 || true)
rm -f "$MDNS_TMP"
if [[ -z "$MDNS_IP" ]]; then
    # Fallback: ping
